(** * LEProps4: Properties of LEval (a big-step semantics for LDef) *)

(**  An attempt at soundness of eval using a [value_has_type]
      relation with a mutually recursive [vapp] clause. 
      It doesn't seem to work in it's current form. *)
...
(*I think this is no good. The TV_Abs rule doesn't work, as the quantification over va is wrong.
    In the proof below, we end up with v2 and va as different variables which won't unify.
*)
...
Hint Constructors value_has_type rtcontext_has_type vapp.

(* ###################################################################### *)
(**  *Lemmas about the forms of values *)
...
Lemma fun_vals: forall v T1 T2, value_has_type v (TArrow T1 T2) ->
  exists xf tf gf, v = vabs xf tf gf (* should there be more? *).
Proof.
  introv H. inversion H. subst. inversion H4. exists xf tf gf. reflexivity.
Qed.

Definition evaluates (t : tm)  (g : rctx)  (erT : ty) : Prop :=
  exists v, eval t g v /\ value_has_type v erT.

(** * tests *)

Example e_not_T_prim: 
    exists vf, eval f_not rctx0 vf /\ 
      forall va, value_has_type va TBool -> (vapp vf va TBool).
Proof.
  eexists. unfold f_not. eauto 15 using bool_vals. split.
    eauto. (* eapply E_Abs. *)
    introv Ha. apply bool_vals in Ha. destruct Ha; (subst; eauto 10). 
Qed.

Example e_not_T: 
(*     evaluates f_not rctx0 (TArrow TBool TBool). *)
    exists v, eval f_not rctx0 v /\ value_has_type v (TArrow TBool TBool).
Proof.
  eexists.  unfold f_not. eauto 15. (*Hint Resolve fun_vals. eauto 10.*)
...
Proof.
  ...
  t_cases (induction t) Case; intros G T Hty g Hg. 
    ...
    Case "tapp". inversion Hty; subst; clear Hty.
      ...
      eexists. split.
        inversion Hv1. subst; clear Hv1. inversion H3. subst; clear H3.
        eapply (E_App t1 t2 _ v2 _ g He1 He2). eapply EA. apply H.
