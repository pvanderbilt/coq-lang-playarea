(** * LEProps2: Properties of LEval (a big-step semantics for LDef) *)

(**  V2: A copy of LEProps1 that hasn't gone anywhere. *)
...
with evaluates_to : tm -> rctx -> ty -> Prop := 
   | TVE : forall t g T, (exists v, t / g || v /\ (value_has_type v T)) -> t / g |: T
...
Hint Constructors evaluates_to.
...
(* Lemma fun_vals0: forall xf tb gc T1 T2,
   ( forall va,  va : T1 -> tb / (acons xf va gc) |: T2 )
    <-> (vabs xf tb gc : TArrow T1 T2).
Proof.
    admit.    (*  !!! *)
Qed.
 *)
...
Proof.
  ...
  t_cases (induction t as [ x | t1 ? t2 ? | x Tx tb | | | ti ? tt ? te ? ]) Case; intros G T Hty g Hg;
    ...
    Case "tvar". eauto using lemma3. 
      (*destruct (lemma3 G i T g Hty Hg) as [v [Hl Hv]]. exists v. auto. *)
    Case "tapp".
      ...
      *)
      ...
      apply TVE. exists vr. 
      exact (conj (E_App t1 t2 _ v2 _ g He1 He2 (EA xf tb gf _ _ Her)) Hvr).
(* With the current definition of value_has_type, the "tabs" case is so trivial it doesn't show up. 
    Case "tabs". (* clear IHt.  The IH is NOT BEING USED*) 
      (* exploring *) apply TVE. eexists. split. eapply E_Abs.
...
Theorem bigstep_soundness' :
  forall (G : context) (t : tm) (T : ty),
    G |- t \in T -> 
      forall (g : rctx), g :* G -> t / g |: T.
Proof.
  intros G t T Hty. has_type_cases (induction Hty) Case; intros g Hg; eauto.
    Case "T_Var". eauto using lemma3.
